...
The script will login and apply the syslog config to the devices in the device_list.txt file.
...
def ios_apply_syslog():
    print '#' * 28 + 'Applying Syslog configuration to %s' % (dev_hostname) + '#' * 29
    net_connect.config_mode()
    output = net_connect.send_config_from_file('templates/SYSLOG')
    print output
    output = net_connect.send_command('write mem')
    print output

def xr_apply_syslog():
    output = net_connect.send_config_from_file('templates/XR-SYSLOG')
    print(output)
